...
def compute_stat_ci(
    ...
    for j in range(J):
        # kernel_X = 1.0 * RBF(length_scale=gwidthPredX * np.ones(dZ), length_scale_bounds=(1e-9, 1e9)) + WhiteKernel( 
        #     noise_level=lam_X, noise_level_bounds=(1e-9, 1e9)
        # )
        ...
        # kernel_Y = 1.0 * RBF(length_scale=gwidthPredY * np.ones(dZ), length_scale_bounds=(1e-9, 1e9)) + WhiteKernel( 
        #     noise_level=lam_Y, noise_level_bounds=(1e-9, 1e9)
        # )
